...
void ball_PID()
{
    ...
    {
        i_x += i_gain * error_x / 10.0;
    }
    else
    {
        i_x = 0;
    }
    if (error_y < 200 && error_y > -200)
    {
        i_y += i_gain * error_y / 10.0;
    }
    else
    {
        i_y = 0;
...
void calc_PID()
{
    ...
    if ( measured_angle_a - set_angle_a < deadband && measured_angle_a - set_angle_a > -deadband)
    {
        set_speed_a = 0;
    }
    if ( measured_angle_b - set_angle_b < deadband && measured_angle_b - set_angle_b > -deadband)
    {
        set_speed_b = 0;
    }
    if ( measured_angle_c - set_angle_c < deadband && measured_angle_c - set_angle_c > -deadband)
    {
        set_speed_c = 0;
    }
}
...
void touch_screen()
{
    ...
    if (touch == 1)
    {
        measure_touch();            // measure touch position if touched
...
void stepper_run(uint8_t motorNumber,float motor_speed)
{
    ...
    {
        PWM_count = 0; // set compare value to 0 to keep stepper output from toggling when zero velocity needed
...
void ball_patterns_and_display()
{
    if (pattern_rate_counter < pattern_rate)
    {
        pattern_rate_counter++;
    }
    else
    {
