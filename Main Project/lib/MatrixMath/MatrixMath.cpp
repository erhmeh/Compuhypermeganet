...
// Uses tabs to separate numbers under assumption printed mtx_type width won't
// cause problems
...
// C = A*B
void MatrixMath::Multiply(mtx_type *A, mtx_type *B, int m, int p, int n,
                          mtx_type *C) {
...
// * NOTE: The argument is ALSO the result matrix, meaning the input matrix is
// REPLACED
int MatrixMath::Invert(mtx_type *A, int n) {
  ...
  for (k = 0; k < n; k++) {
    ...
    for (i = 0; i < n; i++) {
      if (i != k) {
        tmp = A[i * n + k];
        A[i * n + k] =
            0.0f; // The other place where in matrix becomes result mat
        for (j = 0; j < n; j++) {
  ...
  // Done, now need to undo pivot row swaps by doing column swaps in reverse
  // order
  for (k = n - 1; k >= 0; k--) {
    if (pivrows[k] != k) {
      for (i = 0; i < n; i++) {
